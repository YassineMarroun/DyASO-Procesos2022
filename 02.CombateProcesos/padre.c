...
int main(int argc, char *argv[]) {
    ...
    resultado = semctl(semid, 0, SETVAL, 0);
    if (resultadoSem == -1) {
        perror("Error semctl");
        exit(3);
    }
    ...
    // A continuación se crean N procesos hijos, donde N es un parámetro
    // que se pasa como entrada al invocarlo desde Ejercicio2.sh
    // Cada hijo realizará un exec() del ejecutable HIJO.
    for(int i=1; i <= n; i++) {
        pid = fork();
        if (pid == -1) {
            ...
            /* Este código solo lo ejecuta el proceso el proceso padre */
            printf("pid del proceso padre = %n\n", getpid());
        }
    }
